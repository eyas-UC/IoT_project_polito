...
with open('led_config.json', 'r') as settings_file:
    json_data = json.load(settings_file)
settings_file.close()


GPIO.setup(json_data['pin_number'], GPIO.OUT, initial=GPIO.LOW) 
...
class client():
    def __init__(self, clientID, broker, port):
        ...
        self.pin_no= json_data['pin_number']
    ...
    def on_message_r(self, paho_mqtt, userdata, msg):
        ...
        if msg.payload.decode('utf-8') == "ON":
            GPIO.output(self.pin_no, GPIO.HIGH)  # Turn on
            # sleep(1)                  # Sleep for 1 second
        elif msg.payload.decode('utf-8') == "OFF":
            GPIO.output(self.pin_no, GPIO.LOW)  # Turn off
            # sleep(1)                  # Sleep for 1 second
