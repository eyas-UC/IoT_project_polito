...
#called when user choses one of the buttons after sending "/help" / "commands"
    def on_callback_query(self,msg):
        ...
        if self.message == "/switchOn":
            self.bot.sendMessage(self.chatID, text=f'available houses are {self.active_IDs}')
        ...
        #     self.previous_message = self.message
        if self.message == "/houses":
            self.bot.sendMessage(self.chatID, text=f'available {self.houses_list_dict}')
        ...
        if self.message == "/picture01":
            ...
            state, rc_url = service_search.search(sc_url='http://linksmart:8082',service_title='RC')
            if state == True:
                RC_list = resource_search.r_search(sc_url='http://rc:8087')
    ...
    def on_chat_message(self, msg):
        ...
        if message == "/help" or "/commands":
            ...
            buttons2 = [[
                ...
                InlineKeyboardButton(text=f'all', callback_data=f'/houses'),
